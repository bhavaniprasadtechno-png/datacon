...
def get_together_chat_completion(
    ...
    messages: list = None,
    agent_name: str = "Prescriptive Strategy Agent",
    max_tokens: int = 700,
    **kwargs
) -> str:
...
# ============================================================================
# PRESCRIPTIVE & DIAGNOSTIC ANALYSIS ORCHESTRATOR
# ============================================================================
...
class DiagnosticAnalysis:
    ...
    def __init__(
        ...
        self.mode = mode or "prescriptive"
        self.yaml_content = yaml_content
        self.full_schema = full_schema

    def get_diagnostic_spec(self, user_query: str, previous_question: str = "", is_followup: bool = False) -> Dict[str, Any]:
        """
        Generates a small diagnostic/prescriptive specification without sending full YAML schema text.
...
"""
    ...
    def get_prescriptive_spec(self, user_query: str, previous_question: str = "", is_followup: bool = False) -> Dict[str, Any]:
        """Convenience alias for prescriptive specification extraction."""
        return self.get_diagnostic_spec(user_query, previous_question=previous_question, is_followup=is_followup)
    ...
    def perform_llm_business_reasoning(
        ...
        """
        Executes ONE final diagnostic / prescriptive reasoning LLM call over compact evidence.
        """
        is_prescriptive = (mode == "prescriptive")
        compact_json_str = json.dumps(evidence, separators=(",", ":"))

        est_tokens = len(compact_json_str) // 4
        logger.info(f"[Prescriptive/Diagnostic Evidence] Context size: {len(compact_json_str)} chars | Estimated tokens: ~{est_tokens}")
...
"""
        ...
        try:
            reasoning_output = get_together_chat_completion(
                ...
                agent_name="Prescriptive Strategy Agent" if is_prescriptive else "Diagnostic Reasoning Agent",
                max_tokens=700
            )
            out_tokens = len(reasoning_output) // 4
            logger.info(f"[Prescriptive Reasoning] Output size: {len(reasoning_output)} chars | Estimated tokens: ~{out_tokens}")
            logger.info(f"=== [PRESCRIPTIVE / DIAGNOSTIC REPORT OUTPUT] ===\n{reasoning_output}")
            return reasoning_output
        except Exception as e:
            logger.error(f"[DiagnosticAnalysis] Error during final LLM Business Reasoning: {e}")
            if is_prescriptive:
                return f"Prescriptive strategy formulation completed for '{user_query}'. Evaluated key drivers and strategic opportunities."
    ...
    def analyze_and_evaluate(
        ...
        """
        Main Orchestration Workflow for Prescriptive & Diagnostic Analysis.
        ...
        8. ONE final reasoning LLM call.
        """
        effective_mode = mode or self.mode or "prescriptive"

        # 1. Get compact specification
        ...
        # 9. ONE final prescriptive / diagnostic reasoning LLM call
        ...
        return {
            ...
            "prescriptive_analysis_details": spec if effective_mode == "prescriptive" else None,
            "diagnostic_eval_info": evidence,
            "prescriptive_eval_info": evidence if effective_mode == "prescriptive" else None,
            "row_count": row_count,
            "needs_insights": True
        }
    ...
    def run_diagnostic_evaluation(
        ...
        report = self.perform_llm_business_reasoning(
            user_query=da_details.get("reasoning", "Prescriptive Strategic Planning" if effective_mode == "prescriptive" else "Diagnostic Investigation"),
            spec=da_details,
            evidence=evidence,
            mode=effective_mode
        )
        return {
            ...
            "key_findings": [f"Evaluated {len(df) if df is not None else 0} records for strategic breakdown."]
        }

    def run_prescriptive_evaluation(
        self,
        df: pd.DataFrame,
        ...
        """Convenience alias for prescriptive evaluation."""
        return self.run_diagnostic_evaluation(
            df=df,
            da_details=pa_details,
            previous_df=previous_df,
            mode=mode or "prescriptive",
...
# ============================================================================
# AGENT PREP INTERFACE FOR CHAT ROUTER & BACKWARD COMPATIBILITY
# ============================================================================
...
async def prepare(question: str, model: str | None = None) -> AgentPrep:
    """Prepares the prescriptive analytics response for a user query."""
